...
def clear_queue(mongo_client, db_name: str, collection_name: str):
    """
    Supprime tous les documents de la collection fournie.
    
    (utilise clear_collection)
    """
    logger.info(f"Clearing queue: {collection_name}")
    result = clear_collection(mongo_client, db_name, collection_name)
    if result.get("success"):
        logger.info(
            f"Cleared {collection_name}: deleted "
            f"{result.get('deleted_count')}"
        )
        return True
    else:
        logger.error(
            f"Failed to clear {collection_name}: {result.get('error')}"
        )
        return False
...
def run_single_llm_worker(worker_id: int, results: list):
    ...
    try:
        ...
        if not llm_worker_path.exists():
            print(
                f"❌ Worker {worker_id}: LLM worker not found at: "
                f"{llm_worker_path}"
            )
            results[worker_id] = {
                "success": False,
        ...
        if result.returncode == 0:
            ...
            print(
                f"❌ Worker {worker_id}: Failed with exit code "
                f"{result.returncode}"
            )
            results[worker_id] = {
                "success": False,
...
def run_llm_workers(config: RunConfig):
    """Run multiple LLM workers in parallel."""
    print(f"\n{'='*40}")
    print(
        f"🤖 RUNNING {config.num_llm_workers} LLM QUEUE WORKERS IN "
        f"PARALLEL"
    )
    print(f"{'='*40}")
    ...
    try:
        ...
        print(
            f"   Each worker will process items until the queue is "
            f"empty"
        )
        ...
        for i in range(config.num_llm_workers):
            thread = threading.Thread(
                target=run_single_llm_worker, args=(i, results)
            )
        ...
        print(
            f"⏳ Waiting for all {config.num_llm_workers} workers to "
            f"complete..."
        )
        ...
        # Consider it successful if at least one worker completed
        # successfully
        if successful_workers > 0:
            print(
                f"✅ LLM processing completed successfully "
                f"({successful_workers}/{config.num_llm_workers} workers)"
            )
            return True
        else:
...
class Processor:
    ...
    def _load_files(self) -> Dict:
        ...
        try:
            return {
                ...
                "configs": db_read(
                    self.mongo_client, self.db_name, "config"
                )
            }
        except Exception as e:
    ...
    def validate_config(self, config: RunConfig) -> bool:
        """
        Valide que test_id existe en config.
        
        Et que les modèles correspondent exactement.
        """
        try:
            db_config = next(
                (c for c in self.files["configs"]
                 if c.get("test_id") == config.test_id),
                None
            )
            if not db_config:
                logger.error(
                    f"Test ID '{config.test_id}' not found in config "
                    f"collection"
                )
                return False
            ...
            for k, v in expected.items():
                if config_models.get(k) != v:
                    logger.error(
                        f"Model mismatch for {k}. db: "
                        f"{config_models.get(k)} expected: {v}"
                    )
                    return False
            return True
        except Exception:
    ...
    def run_initialization(self, config: RunConfig):
        ...
        Lance batch_loop pour créer les iterations initiales.
        """
        if not self.validate_config(config):
            raise RuntimeError("Invalid configuration")

        db_config = db_find_one(
            self.mongo_client,
            self.db_name,
            "config",
...
def llm_worker_loop(config, processor, stop_event):
    ...
    Continuously checks the input queue and processes items until idle
    ...
    while not stop_event.is_set():
        has_items = queue_has_items(
            processor.mongo_client, "poc-llm-processor", "jobs"
        )
        ...
        if has_items:
            ...
            if not ok_workers:
                logger.warning(
                    "No LLM worker succeeded; continuing to next cycle"
                )
        else:
            idle_rounds += 1
            logger.info(
                f"No items in jobs collection (idle round "
                f"{idle_rounds}/{max_idle_rounds_before_exit})"
            )
            if idle_rounds >= max_idle_rounds_before_exit:
                logger.info(
                    "No activity for several cycles — stopping LLM "
                    "worker loop."
                )
                stop_event.set()
                break
...
def output_reader_loop(processor, stop_event):
    """
    Parallel loop for reading output queue.
    
    Continuously reads and processes operations from the output queue.
    ...
    while not stop_event.is_set():
        try:
            operations = read_output_queue_operations(
                processor.mongo_client
            )
            if operations:
                logger.info(
                    f"Processing {len(operations)} operations from jobs "
                    f"collection"
                )
...
def main():
    ...
    connection_string = (
        ...
        or keyring.get_password("mongodb", "client-app")
    )
    if not connection_string:
        logger.error(
            "MongoDB connection string not found (env or keyring)"
        )
        sys.exit(1)
