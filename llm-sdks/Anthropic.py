...
Implementation for Anthropic's Claude API using the official Anthropic Python SDK.
"""
...
class AnthropicSDK(BaseLLMSDK):
    """Anthropic Claude SDK implementation with streaming support."""
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        - deployment: Model identifier (e.g., "claude-3-7-sonnet-20250219")
        """
        ...
        if missing_fields:
            raise ValueError(
                f"Anthropic requires the following fields: {', '.join(missing_fields)}"
            )
    ...
    def complete(
        ...
        Uses streaming to avoid the 10-minute timeout for long requests.
