...
class ChatCompletionsClientSDK(BaseLLMSDK):
    ...
    def validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if missing_fields:
            raise ValueError(
                f"ChatCompletionsClient requires the following fields: {', '.join(missing_fields)}"
            )
    ...
    def complete(
        ...
        Args:
            config: Model configuration with endpoint, apiVersion, and deployment
        ...
        if response_text is None:
            raise RuntimeError("ChatCompletionsClient returned empty response content.")
