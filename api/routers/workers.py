...
async def create_worker(
    ...
    - Admin must provide client_id header to associate worker
...
async def create_workers_batch(
    ...
    - Admin must provide client_id header to associate workers
    ...
    try:
        ...
        if request.group:
            ...
            for worker_id in request.workerIds:
                try:
                    worker = service.get_worker_by_id(
                        worker_id,
                        client_id=client_id,
                        is_admin=is_admin
                    )
        ...
        for worker in workers_to_update:
            ...
            try:
                ...
                if not updated_worker:
                    updated_worker = service.get_worker_by_id(
                        worker_id,
                        client_id=client_id,
                        is_admin=is_admin
                    )
...
async def get_worker(
    ...
    try:
        service = get_worker_service()
        worker = service.get_worker_by_id(
            worker_id, client_id=client_id, is_admin=is_admin
        )
