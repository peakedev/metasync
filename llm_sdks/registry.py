...
class SDKRegistry:
    ...
    Automatically discovers and loads all SDK implementations from the
    llm-sdks directory.
    """
    ...
    def _initialize(cls):
        ...
        Scans the llm-sdks directory for Python files (excluding
        base_sdk.py, registry.py, and __init__.py) and automatically
        imports and registers any classes that inherit from BaseLLMSDK.
        """
        if cls._initialized:
            return
        ...
        for file_path in sdk_dir.glob("*.py"):
            ...
            try:
                module = importlib.import_module(
                    f'llm_sdks.{module_name}'
                )

                # Find all classes that inherit from BaseLLMSDK
                for name, obj in inspect.getmembers(
                    module, inspect.isclass
                ):
                    # Check if subclass of BaseLLMSDK (not itself)
                    if (issubclass(obj, BaseLLMSDK) and
                            obj is not BaseLLMSDK):
                        # Instantiate the SDK
                        sdk_instance = obj()
                ...
                msg = f"Warning: Failed to load SDK from {filename}: {e}"
                print(msg)
                continue
    ...
    def get_sdk(cls, name: str) -> Optional['BaseLLMSDK']:
        ...
        Args:
            name: SDK name (e.g., "ChatCompletionsClient",
                  "AzureOpenAI", "Anthropic", "test")
